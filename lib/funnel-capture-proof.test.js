...
test('default text-back uses centralized business-level Growth Call copy', () => {
  const previousTemplate = process.env.TEXTBACK_SMS;
  delete process.env.TEXTBACK_SMS;
  try {
    assert.match(buildTextBack('Evan'), /Rushes Media received your request/);
    assert.match(buildTextBack('Evan'), /Schedule a 30-minute Growth Call here/);
    assert.doesNotMatch(buildTextBack('Evan'), /Grab 20 min here/);
    assert.doesNotMatch(buildTextBack('Evan'), /it'?s Evan at Rushes|I’ll|I'll/);
  } finally {
