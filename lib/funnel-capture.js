...
// EVAN: this copy is now set (you authorized it) — tweak the wording anytime, but keep
// the STOP opt-out for A2P. Link defaults to the branded /call → your GHL Growth Call widget.
const BOOKING_LINK = process.env.RUSHES_BOOKING_LINK || 'https://rushesmedia.com/call';
const DEFAULT_TEXTBACK =
  `Hey {{firstName}}, it's Evan at Rushes — got your request, thanks for reaching out. ` +
  `Grab 20 min here and I'll walk you through it: ${BOOKING_LINK} ` +
  `(or reply with a time that works). Reply STOP to opt out.`;

function buildTextBack(firstName) {
  const template = process.env.TEXTBACK_SMS || DEFAULT_TEXTBACK;
  return template.replace(/\{\{\s*firstName\s*\}\}/g, firstName || 'there');
}
...
async function captureFunnelLead(raw) {
  ...
  try {
    await ghlRequest(TOKEN, 'POST', `/contacts/${contactId}/tasks`, {
      ...
      body: `Growth Call request from /${input.niche}/. ${input.need ? `Said: "${input.need}"` : ''} Phone: ${input.phone}${textBackSent ? ' · auto text-back sent' : ' · NO consent → no text-back, call them'}`,
