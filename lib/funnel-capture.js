...
function validate(raw) {
  ...
  if (raw && typeof raw.website === 'string' && raw.website.trim()) return { spam: true };

  const checked = validateFields(raw, {
    name: 200, business: 200, phone: 32, email: 254, need: 1000,
    niche: 40, source_url: 300, proof_id: 83, website: 300, calendarUrl: 300,
  }, ['name', 'business', 'phone', 'email', 'need']);
  if (checked.error) return checked;
  for (const field of ['sms_consent', 'sms_consent_care', 'sms_consent_promo']) {
    if (raw[field] !== undefined && !['yes', 'no', true, false].includes(raw[field])) {
      return { error: 'Check the highlighted fields.', fieldErrors: { [field]: 'Choose whether to receive texts.' } };
    }
  }
  raw = checked.values;
...
async function captureFunnelLead(raw) {
  ...
  if (!contactId) throw Object.assign(new Error('Contact upsert returned no contact'), { upstream: true });
  ...
  // Send the acknowledgement only after required capture operations succeed.
  // Consent-gated + non-fatal — a send failure never breaks capture.
  let textBackSent = false;
  let textBackStatus = hasSmsConsent(raw) ? 'failed' : 'not_consented';
  if (hasSmsConsent(raw)) {
    try {
      await sendInstantTextBack(contactId, input.firstName);
      textBackSent = true;
      textBackStatus = 'accepted';
    } catch (err) {
      textBackStatus = err.acceptanceUnknown || ['TimeoutError', 'AbortError'].includes(err.name) ? 'unknown' : 'failed';
      console.error('textback-sms:', textBackStatus);
    }
  }
  ...
  try {
    await ghlRequest(TOKEN, 'POST', `/contacts/${contactId}/tasks`, {
      ...
      body: `Growth Call request from /${input.niche}/.${input.proofId ? ` Proof: ${input.proofId}.` : ''} ${input.need ? `Said: "${input.need}"` : ''} Phone: ${input.phone}${textBackStatus === 'accepted' ? ' · text-back accepted by provider; delivery unconfirmed' : textBackStatus === 'not_consented' ? ' · No SMS consent; call them' : ` · SMS ${textBackStatus}; consent present; call them`}`,
  ...
  return { ok: true, contactId, opportunityId: opportunity?.id, tags, textBackSent, textBackStatus };
}

module.exports = { buildTextBack, captureFunnelLead, extractProofId, validate };
