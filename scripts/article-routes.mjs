// Add an entry alongside each reviewed Markdown article. Main-branch merges publish.
// lastmod is the verified date of a significant content revision, never build time.
export const ARTICLES = [
  {
    slug: 'landing-page-or-full-website',
    lastmod: '2026-09-18',
    title: 'Landing page or full website: which does your business need?',
    description: 'Choose the right scope for your website using your offer, traffic sources, buyer questions and the work needed to handle inquiries.',
    category: 'Web',
  },
  {
    slug: 'turning-business-reputation-into-brand-content',
    lastmod: '2026-09-18',
    title: 'Turn your business reputation into useful brand content',
    description: 'Use genuine customer questions, feedback and work examples to plan photographs, videos and website content that help people choose your business.',
    category: 'Brand Media',
  },
  {
    slug: 'planning-a-brand-photo-and-video-shoot',
    lastmod: '2026-09-23',
    title: 'How to plan a brand photo and video shoot',
    description: 'Build a useful shoot brief: decide where the assets will appear, what to capture, who approves them and how the finished files will be delivered.',
    category: 'Brand Media',
  },
  {
    slug: 'building-a-lead-follow-up-process',
    lastmod: '2026-09-09',
    title: 'How to build a lead follow-up process your team can run',
    description: 'Review recent inquiries, assign a clear owner and next step, and decide which follow-up tasks need a person, a reminder or automation.',
    category: 'Business Systems',
  },
  {
    slug: 'hiring-a-google-ads-agency',
    lastmod: '2026-09-26',
    title: 'What to check before hiring a Google Ads agency',
    description: 'A practical checklist for business owners: Google Ads account access, measurement, fees, campaign management, landing pages and the handoff after an inquiry.',
    category: 'Campaigns',
  },
];
